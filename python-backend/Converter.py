...
def translation(lines,font_data):
    ...
    for i in range(0,len(lines)):
        ...
        if y[-1] not in layer_height:
            layer_height[y[-1]] = get_text_size(draw, "a", font)[1] + font_size 
        ...
        if line == "START" or line == "STOP":
            ...
            if get_text_size(draw, line, font)[0] + font_size * 2 > branch_width[x[-1]]:
                branch_width[x[-1]] = get_text_size(draw, line, font)[0] + font_size * 2

            if get_text_size(draw, line, font)[1] + font_size + 2*font_size > layer_height[y[-1]]:
                layer_height[y[-1]] = get_text_size(draw, line, font)[1] + 2*font_size
            ...
            if get_text_size(draw, line, font)[1] + font_size > layer_height[y[-1]]:
                layer_height[y[-1]] = get_text_size(draw, line, font)[1] + font_size
            ...
            if len(line) > len("IF")+1:    
                width = 3/2*get_text_size(draw, line, font)[0]
                height = 5*get_text_size(draw, line, font)[1] 
            else:
                width = 5*font_size
                height = 5*font_size 
...
def drawer(chart_code,max_branch,max_y,layer_height,branch_width,font_data):
    ...
    def Terminator(text,position):
        ...
        width = get_text_size(draw, text, font)[0] + font_size * 2
        height = get_text_size(draw, text, font)[1] + font_size * 2
    ...
    def Decision(text,position):
        ...
        if len(text) > len("IF")+1:    
            width = 3/2*get_text_size(draw, text, font)[0]
            height = 5*get_text_size(draw, text, font)[1] 
            text_offset = get_text_size(draw, text, font)[0]/4
            
        else:
            width = 5*font_size
            height = 5*font_size
            text_offset = (width - get_text_size(draw, text, font)[0])/2
    ...
    def Process(text,position):
        ...
        width = get_text_size(draw, text, font)[0] + font_size * 2
        height = get_text_size(draw, text, font)[1] + font_size
    ...
    for i in range(0,len(chart_code)):
        ...
        if block['type'] == 'IO':
            ...
            height = get_text_size(draw, "c", font)[1] + font_size
